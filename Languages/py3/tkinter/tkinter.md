...
- Projects (Ideas)
    - [SlideShow](#slideshow): An image slideshow presenter.
    - [BlackJack](#blackjack): A single-player blackjack game.
    - [Form Fillup](): A form-type bilingual data entry application.
    - [Text Editor](#texteditor): A customizable text-editor.
    - [Color Palette](): A HEX-value chart for common RGB colors.
        ...
        - Right click anywhere, choose `View color (RGB-HEX)` to get HEX values of that pixel.
    - To-Do list.
...
- __Number cards__ are worth their face value, __Picture cards__ are worth 10, and an __Ace__ is worth either 1 or 11 (1, if adding 11 to the total value of the cards in hand takes it beyond 21). Players are initially dealt two cards and can either choose to hit (receive another card) or stick (submit their current hand). <br>
...
- [x] Classes - Card, Deck, Hand, Game. A deck initializes 52 cards. A hand receives cards dealt from a shuffled deck.
...
> CHANGES: GUI Version (No Sound). <br>

- [x] Convert CLI to GUI.
- [x] Insert cards. Obtained from [here](https://opengameart.org/content/playing-cards-0).
- [x] Resize card images to fit window. Resized using `magick mogrify -resize 120x165 *.png`.
...
> CHANGES: Added betting options & audio to GUI. <br>

- [ ] Generate animations on canvas.
- [ ] Control GUI when animation is playing.
- [ ] Insert sounds using `pygame` module. Obtained from [here](https://opengameart.org/content/54-casino-sound-effects-cards-dice-chips).
- [x] Insert link to video. Added using `[![alt_text](img_link)](video_link)`.
...
[![Video Here](resources/PRJ02b.gif)](resources/PRJ02c.mp4)
